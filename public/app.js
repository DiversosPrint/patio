...
async function enter(user) {
  state.user = user; $('#userName').textContent = user.name; $('#userRole').textContent = user.role; document.querySelectorAll('.admin-backup').forEach(el => el.classList.toggle('hidden', user.role !== 'admin')); $('#loginView').classList.add('hidden'); $('#appView').classList.remove('hidden'); await load();
}
